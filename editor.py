...
# FIX: Changed render_resort_grid back to render_resort_selector to match your library
        ...
        for season in year_obj.get("seasons", []):
            for cat in season.get("day_categories", {}).values():
                if isinstance(rp := cat.get("room_points", {}), dict):
                    # FIX: Safely add as string to avoid TypeError during sort
                    for k in rp.keys():
    ...
    if not base_holidays:
        ...
        for h_idx, h in enumerate(base_holidays):
            ...
            with st.expander(f"🎊 {disp_name}", expanded=False):
                ...
                edited_df = st.data_editor(
                    df_pts,
                    key=rk(resort_id, "holiday_points_input", base_year, h_idx),
                    use_container_width=True,
                    hide_index=True,
                    column_config={
                        "Room Type": st.column_config.TextColumn(disabled=True),
        ...
        if n_days > 0:
            for room in room_types:
                if room in rp and rp[room] is not None:
                    # FIX: Handle string vs int issues safely
                    try:
                        pts = int(rp[room])
                    except:
                        pts = 0
...
def validate_resort_data_v2(
    ...
    for year in years:
        ...
        for season in year_obj.get("seasons", []):
            ...
            if not season.get("periods"):
                issues.append(f"[{year}] Season '{sname}' has no dates defined.")
    return issues
