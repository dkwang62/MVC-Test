...
# FIX: Using the correct function name found in your ui.py
    ...
    with st.sidebar.expander("🔀 Merge", expanded=False):
        ...
        if merge_upload:
            try:
                ...
                merge_data = sanitize_loaded_data(merge_data)
            ...
            for key, cat in dc.items():
                ...
                if st.button("Save Changes", key=rk(resort_id, "save_master_rp", base_year, s_idx, key)):
                    if not edited_df.empty:
                        ...
                        sync_season_room_points_across_years(working, base_year) # Force sync immediately
        ...
        if n_days > 0:
            for room in room_types:
                if room in rp and rp[room] is not None:
                    # FIX: Handle string/int conversion safely
                    try:
                        pts = int(rp[room])
                    except:
                        pts = 0
