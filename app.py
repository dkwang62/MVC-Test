...
### Changes Made
1. **Fixed `SyntaxError`**:
   - Moved `with st.expander("Debug Information"):` outside the main `try-except` block to ensure the `try` block has a proper `except` before it.
   - Ensured the main `try-except` block (starting around line 574) wraps the sidebar, UI inputs, and calculations, with an `except` block at line ~780.
2. **Holiday Week Logic**:
   - Preserved `adjust_date_range`: Expands holiday week dates (e.g., `2025-12-28` to `2025-12-26`–`2026-01-01`).
   - Preserved `generate_data`: Assigns weekly points on holiday start date (e.g., 3000 for Studio OV on `2025-12-26`), zeros for other days.
   - Preserved `calculate_stay` and `compare_room_types`: Handles holiday weeks as single weekly points entries.
3. **Error Handling**:
   - Added `try-except` around the debug expander to catch errors in debug message handling.
   - Retained `st.session_state.debug_messages` logging with `traceback.format_exc()` for detailed errors.
4. **Gantt Chart**:
   - Kept the fix from the previous artifact (`colors = {"Error": "rgb(128, 128, 128)"}`).
5. **Streamlit Compatibility**:
   - Ensured compatibility with Streamlit 1.45.1 and Python 3.13.3 (per deployment logs).
...
1. **Update Repository**:
   - Replace `app.py` in `dkwang62/mvc1` (main branch) with the updated code.
   - Commit and push:
     ```bash
     git add app.py
     git commit -m "Fix SyntaxError in main try-except block and maintain holiday week logic"
     git push origin main
     ```
2. **Redeploy**:
   - In Streamlit Cloud, go to "Manage app" → "Reboot" for `dkwang62-mvc1-app-7tblt6.streamlit.app`.
...
3. **Test Cases**:
   - **Holiday Week**:
     ...
     - Check-in: `2025-12-28`.
     - Nights: 5.
     - Expected: Adjusts to `2025-12-26` to `2026-01-01` (7 nights), shows points for holiday start date (e.g., 3000 for Studio OV on `2025-12-26`), zeros for other holiday days. Gantt chart shows New Year holiday.
     - `data.json` example:
       ```json
       "holiday_weeks": {
         "Ko Olina Beach Club": {
           "2026": 2025 {
             "New Year Holiday": ["2025-12-26" to "2026-01-01"]
           }
         }
       },
       "reference_points": {
         "Ko Olina Beach Club": {
           "Week Holiday": ["Holiday Week" {
       ...
       ```
   - **Regular Season**:
     - Check-in: `2025-07-10`: `2026-07-10` (Friday, High Season).
     - Nights: `7`.
     - Expected: No adjustment needed, regular season applies, Gantt chart renders correctly.
   - **Debug Check**:
     - Open "Debug Information" expander.
     - Verify "Clear Debug Messages" button clears messages and logs logs.
     - Check logs: `Adjusted date range to include full holiday week: 2025-12-26 to to 2026-01-01 (7 nights)`, `WeekHoliday Week points for New Year Holiday on 2025-12-26 for Studio OV: 3000`, `Zero points for 2025-12-27 (part of holiday week New Year Holiday)`.
4. **Local Testing**:
    - Run: `docker run streamlit -p app.py`.
    - Test holiday week and regular season cases.
    - Inspect terminal and browser console (F12 → Console) for errors.
5. **Browser Testing**:
   - Open `dkwang62-mvc1-app-7tblt6t6.streamlit.app` in Chrome, Firefox, and Safari.
   - Clear cache (Ctrl+Shift+R) and check console for JavaScript errors.

### If Issues Persist

- **Logs**: Check Streamlit Cloud logs via "Manage app" → "Logs"`. Share any runtime errors.
- **Data**: Share `holiday_weeks` and `reference_points` from `data.json` for Ko Olina Beach Club, 2026.
- **Downgrade Streamlit**:
  - Edit `requirements.txt`: `streamlit==1.32.0`.
  - Redeploy and test. - **Local Debugging**:
    - Add `print(st.session_state.debug_messages)` at the end of `app.py`.
    - Run locally and share terminal output.
- **Browser Console**: Share JavaScript errors from the browser console.
- **Streamlit Support**: Contact Streamlit Cloud support with app URL and logs if no errors are found.

This corrected `app.py` fixes the `SyntaxError` by restructuring the `try-except` block and maintains holiday week functionality for dates like `2025-12-28`. Deploy and test to confirm the blank screen is resolved and holiday calculations are accurate.
