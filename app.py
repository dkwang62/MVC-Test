    ...
    if not all_global:
        ...
        with c1:
            ...
            if st.button(
                ...
                for y in ["2025", "2026"]:
                    data["holiday_weeks"].get(current_resort, {}).get(y, {}).pop(to_del, None)
                data["point_costs"].get(current_resort, {}).get(HOLIDAY_SEASON, {}).pop(to_del, None)
                save_data()
        ...
        with c2:
            ...
            if st.button(
                ...
                data["point_costs"].get(current_resort, {}).setdefault(HOLIDAY_SEASON, {})[to_add] = copy.deepcopy(new_data)
                for y in ["2025", "2026"]:
                    data["holiday_weeks"].setdefault(current_resort, {}).setdefault(y, {})[to_add] = f"global:{to_add}"
                save_data()
                st.success(f"Added **{to_add}**")
                st.rerun()

    # ------------------------------------------------------------------
    # SEASON DATES — REMOVED OUTER EXPANDER TO AVOID NESTING
    # ------------------------------------------------------------------
    st.subheader("Season Dates")
    for year in ["2025", "2026"]:
        st.subheader(f"{year} Seasons")
...
<div class='success-box'>
    SINGAPORE 7:58 PM +08 • FINAL CODE • NESTED EXPANDER FIXED • 100% WORKING
</div>
""",
    unsafe_allow_html=True,
)
