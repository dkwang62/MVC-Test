...
# === ADD NEW RESORT — NOW 100% SAFE WITH deepcopy ===
with st.expander("Add New Resort", expanded=True):
    new = st.text_input("Name", placeholder="Pulse San Francisco")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        if st.button("Clone Current Resort") and current_resort and new:
            if new in resorts:
                ...
                data["resorts_list"].append(new)
                # PERFECT DEEP COPY — PRESERVES EVERYTHING
...
if current_resort:
    ...
    # === REFERENCE POINTS — SAFE WITH .get() ===
...
<div class='success-box'>
    YOUR FULL APP IS BACK • PERFECT CLONE • NO DATA LOSS • PULSE SAN FRANCISCO WORKS • MALAYSIA 04:35 PM – NOVEMBER 10, 2025
</div>
