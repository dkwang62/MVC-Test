"""
MVC Calculator v2.0 - Optimized Architecture + Full Comparison Feature
"""
...
# ====================================================================
# CALCULATION ENGINE (extended with comparison)
# ====================================================================
class CalculationEngine:
    ...
    def calculate_rental_stay(self, resort_name: str, room_type: str, checkin: datetime.date,
        ...
        for i in range(nights):
            ...
            if daily.holiday and daily.holiday.is_start(date):
                ...
                total_rent += rent
                continue
            if current_holiday:  # skip middle holiday days
                continue
            rows.append({
    ...
    def compare_rentals(self, resort_name: str, room_types: List[str],
        ...
        """Return pivot table + chart data for multiple room types"""
        rows = []
        daily_chart = []
        holiday_chart = []
        ...
        for i in range(nights):
            date = checkin + timedelta(days=i)
            daily = self.repo.get_daily_points(resort_name, date)
            is_holiday_start = daily.holiday and daily.holiday.is_start(date)
            if is_holiday_start:
                h = daily.holiday
                holidays_seen.add(h.name)
                ...
                for room in room_types:
                    ...
                    rent = math.ceil(raw * rate_per_point)
                    rows.append({
                        "Date": f"{h.name} ({start_str} - {end_str})",
                        "Room Type": room,
                        "Rent": f"${rent}"
                    })
                ...
                continue
            if daily.holiday:  # middle of holiday → skip
                continue
            for room in room_types:
                raw = daily.get_points(room)
                rent = math.ceil(raw * rate_per_point)
        ...
        daily_df = pd.DataFrame(daily_chart)
        holiday_df = pd.DataFrame(holiday_chart)
       
        return pivot, daily_df, holiday_df
...
# ====================================================================
# UI & GANTT (unchanged + comparison added)
# ====================================================================
def setup_page():
...
def main():
    ...
    with st.sidebar:
        ...
        # (same config code as before – omitted for brevity)
        # ... [your existing sidebar code here] ...
        # (keep exactly what you already have for Owner/Renter options)

    # Resort selection (unchanged)
    cols = st.columns(6)
    ...
    # === SINGLE ROOM CALCULATION (unchanged) ===
    if user_mode == "Renter":
        ...
        # Owner calculation (your existing code)
        pass
   
    # === COMPARISON SECTION (FULLY RESTORED) ===
    if compare:
        ...
        try:
            ...
            if not total_row.empty:
                ...
                fig = px.bar(total_df, x="Room Type", y="Total Rent ($)", color="Room Type",
                            text="Total Rent ($)", text_auto=True, height=500)
        ...
        if not daily_df.empty:
            ...
            fig.update_layout(bargap=0.2, legend_title="Room Type")
            st.plotly_chart(fig, use_container_width=True)
