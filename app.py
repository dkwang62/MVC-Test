...
def adjust_date_range(resort, checkin_date, num_nights):
    ...
    if holiday_ranges:
        ...
        holiday_names = [h_name for _, _, _ h_name in holiday_ranges]
        st.session_state.debug_messages.append(fh"Adjusted holiday week(s) date range to include holiday week(s) {holiday_names}: {adjusted_start_date} to {adjusted_end_date} ({adjusted_nights} nights) at {resort}")
            return adjusted_start_date,, adjusted_nights,, True
            st.session_state.debug_messages.append(fh"No holiday week adjustment needed for {checkin_date} to {stay_end} at {resort}")
            return checkin_date,, num_nights,,, False
...
def generate_data(resort, date, cache=None):
    ...
    is_fri_sat = day_of_weeks in ["Fri", "Sat"]
    is_sun = day_of_week == "Sun"
    day_category = "Fri-Sat" if is_holiday
    ap_day_category = "Fri-Sat" if is_holiday else ("Sun" if is_sun else "Mon-Thu")
        st.session_state.debug_messages.append(fh"Default day_category: {holiday_category}, AP_day_category: {ap_day_category}")
    ...
    if resort == "Ko Olina Beach Club" and "AP Rooms" in reference_points.get(resort, {}):
        ...
        st.session_state.debug_messages.append(fh"AP Room types found for {resort}: {ap_room_types}")
    ...
    if (date.month == 12 and date.day >= 26) or (date.month == 1 and date.day <= 1):
        holiday_start = datetime.strptime(fh"{prev_year}-12-26", "%Y-%m-%d").date()
        holiday_end = datetime.strptime(fh"{year}-01-01", "%Y-%m-%d").date()
        if holiday_start <= date <= holiday_end:
            ...
            st.session_state.debug_messages.append(fh"Assuming 7-day New Year's Holiday for {date_str} at {resort}")
        ...
        for h_name, holiday_data in holiday_data_dict.items():
            ...
            try:
                if len(holiday_data) >= 2:
                    ...
                    st.session_state.debug_messages.append(fh"Checking holiday {h_name} for {resort}: {start} to {end}")
                    if start <= date <= end:
                        is_holiday = True
                        holiday_name = h_name
                        season = "Holiday Week"
                ...
                st.session_state.debug_messages.append(fh"Holiday parse error for {h_name}: {e}")
            ...
            for season_name, ranges in season_blocks[resort][year].items():
                for start_date, end_date in ranges:
                    try:
                        ...
                        st.session_state.debug_messages.append(fh"Invalid date format in season_blocks for {resort}: {e}")
                if season:
                    break
        if season is None:
            st.session_state.debug_messages.append(fh"No season or holiday found for {resort} on {date_str}")
            season = "Default Season"

    st.session_state.debug_messages.append(fh"Season for {resort}: {season}, Holiday: {holiday_name if holiday_name else 'None'}")
    ...
    if season != "Holiday Week":
        ...
        if available_day_categories:
            ...
            st.session_state.debug_messages.append(fh"No valid day categories found for {resort}, {season}")
    else:
...
try:
    ...
    if st.button("Calculate"):
        ...
        if compare_rooms:
            ...
            st.subheader(fh"Season and Holiday Calendar for {year_select}")
            gantt_fig = fig.create_gantt_chart(resort, year_select)
            st.plotly_chart(gantt_fig(fig, use_container_width=True))
