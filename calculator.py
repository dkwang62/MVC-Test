    ...
    def calculate_breakdown(
        ...
        while i < nights:
            ...
            if holiday and holiday.name not in processed_holidays:
                ...
                row = {
                    "Date": d.strftime("%Y-%m-%d"), "Day": d.strftime("%a"), "Points": eff
                }
                if is_owner:
                    if owner_config.get("inc_m", False):
                        row["Maintenance"] = m
...
def load_user_settings(uploaded_file):
    ...
    try:
        ...
        # Map simple keys to session state keys
        ...
        # Robust Tier Mapping (Handle variations in string)
...
def main() -> None:
    ...
    if "pref_salvage_value" not in st.session_state: st.session_state.pref_salvage_value = 3.0 # Default $3
    if "pref_useful_life" not in st.session_state: st.session_state.pref_useful_life = 10 # Default 10 years
    ...
    with st.sidebar:
        ...
        if mode == UserMode.OWNER:
            ...
            # --- INPUT WIDGETS: KEY ONLY (No 'value' to avoid conflict) ---
            ...
            # --- RADIO WIDGET: KEY ONLY (No 'index') ---
            ...
            with st.expander("🔧 Advanced Options", expanded=False):
                ...
                if inc_c or inc_d:
                    ...
                    # Key only, disabled state dependent on checkboxes
            ...
            # RENTER MODE (Uses defaults, no session keys needed for inputs unless persisted)
        ...
        # IMPORTANT: Pass the CORRECT RATE from the session state (or widget) to the calculator
        # If we are in owner mode, 'rate' variable was updated by the widget above.
    ...
    st.divider()
    # Correctly pass the rate for owner mode
    if mode == UserMode.OWNER:
        calc_rate = st.session_state.pref_maint_rate
    else:
        calc_rate = rate # use the renter rate defined earlier
