...
class MVCCalculator:
    ...
    def calculate_breakdown(
        ...
        while i < nights:
            ...
            # --- HOLIDAY BLOCK (whole holiday treated as one row) ---
    ...
    def compare_stays(
        ...
        for room in rooms:
            ...
            chart_rows.append(
                {"Room Type": room, "Cost": res.financial_total}
            )

        return ComparisonResult(
            pivot_df=pd.DataFrame(pivot_rows),
            daily_chart_df=pd.DataFrame(chart_rows),
            holiday_chart_df=pd.DataFrame(),  # Not used in this simplified version
        )
...
def main() -> None:
    ...
    # Owner preferences (persist between runs)
    ...
    # Renter preferences (separate from owner)
    ...
    with st.sidebar:
        ...
        if mode == UserMode.OWNER:
            ...
            rate = owner_rate  # used in calculation

            # Owner discount tier (separate state)
            owner_tier = st.radio(
                "Discount Tier:",
                TIER_OPTIONS,
            ...
            # Map owner_tier → discount policy and multiplier
    ...
    # Holiday adjustment only after user has actively set a date
    ...
    # Room types available for this date
    ...
    if mode == UserMode.OWNER:
        ...
        if res.discount_applied:
            st.success(
                f"Discount applied on {len(res.discounted_days)} day(s)."
            )
